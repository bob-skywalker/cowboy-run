...
class ScorePopupNode: PopupNode {
    ...
    func addStars(){
        for i in 0...2{
            let empty = SKSpriteNode(imageNamed: GameConstants.StringConstants.starEmpty)
            let star = SKSpriteNode(imageNamed: GameConstants.StringConstants.starFull)
    ...
    func animateStar(number: Int){
        let star = self[GameConstants.StringConstants.starFull + "_\(number)"].first!
